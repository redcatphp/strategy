**No longer actively maintained. I work now with NodeJS and I recommand you to take a look at [di-ninja](https://github.com/di-ninja/di-ninja), it's the same paradigm ported to Javascript**
...
1. [Notes about Strategy](http://redcatphp.com/strategy-dependency-injection#about)
	1. [Origin](http://redcatphp.com/strategy-dependency-injection#origin)
	2. [Differences](http://redcatphp.com/strategy-dependency-injection#differences)
2. [Paradigm](http://redcatphp.com/strategy-dependency-injection#paradigm)
	1. [Simplify your code](http://redcatphp.com/strategy-dependency-injection#simplify)
	2. [Improve maintainability](http://redcatphp.com/strategy-dependency-injection#maintainability)
	3. [Improve scalability](http://redcatphp.com/strategy-dependency-injection#scalability)
	4. [Be Wise](http://redcatphp.com/strategy-dependency-injection#be-wise)
3. [Get Started](http://redcatphp.com/strategy-dependency-injection#get-started)
	1. [Di instanciation](http://redcatphp.com/strategy-dependency-injection#di-instanciation)
	2. [Classes instanciation](http://redcatphp.com/strategy-dependency-injection#classes-instanciation)
4. [Basic usage](http://redcatphp.com/strategy-dependency-injection#basic-usage)
	1. [Object graph creation](http://redcatphp.com/strategy-dependency-injection#basic-usage-1)
	2. [Provistrategy additional arguments to constructors](http://redcatphp.com/strategy-dependency-injection#basic-usage-2)
5. [Shared dependencies](http://redcatphp.com/strategy-dependency-injection#shared-dependencies)
	1. [Using rules to configure shared dependencies](http://redcatphp.com/strategy-dependency-injection#shared-dependencies)
6. [Configuring the container with rules](http://redcatphp.com/strategy-dependency-injection#config-rules)
	1. [Substitutions](http://redcatphp.com/strategy-dependency-injection#config-rules-substitutions)
	2. [Inheritance](http://redcatphp.com/strategy-dependency-injection#config-rules-inheritance)
	3. [Constructor Parameters](http://redcatphp.com/strategy-dependency-injection#config-rules-constructor)
	4. [Setter injection (mutators)](http://redcatphp.com/strategy-dependency-injection#config-rules-mutators)
	5. [Default rules](http://redcatphp.com/strategy-dependency-injection#config-rules-default)
	6. [Named instances](http://redcatphp.com/strategy-dependency-injection#config-rules-named)
	7. [Sharing instances across a tree](http://redcatphp.com/strategy-dependency-injection#config-rules-tree)
7. [Rule cascastrategy](http://redcatphp.com/strategy-dependency-injection#cascastrategy-rules)
8. [Arbitrary Data](http://redcatphp.com/strategy-dependency-injection#arbitrary-data)
	1. [Simple variable definition](http://redcatphp.com/strategy-dependency-injection#simple-variable)
	2. [Anonymous functions](http://redcatphp.com/strategy-dependency-injection#anonymous-functions)
	3. [Defining factories manually](http://redcatphp.com/strategy-dependency-injection#manual-factory)
	4. [Protecting anonymous functions](http://redcatphp.com/strategy-dependency-injection#protecting-anonymous)
	5. [Extend definitions](http://redcatphp.com/strategy-dependency-injection#extend-definitions)
9. [PHP Config](http://redcatphp.com/strategy-dependency-injection#php-config)

1. Notes about Strategy
----------------------

### 1.1 Origin

 Strategy is mainly inspired by [Dice](https://r.je/dice.html) with added [Pimple](http://pimple.sensiolabs.org) abilities and great improvements.
...
- freeze config optimisation
 ...
 Many chapters of the following documentation correspond to [Dice documentation](https://r.je/dice.html) with some modifications, reformulations, additions and new features explanations. The main differences from Dice and new features explanations will be foreword by Strategy specificity label.
...
### 2.2 Improve maintainability
 ...
 Instead of finstrategy everywhere that *C* is created and have to passing an instance of X to it, this is handled automatically by the [IoC](https://en.wikipedia.org/wiki/Inversion_of_control) Container.  
...
--------------
 ...
 Why is Strategy (and its [Dice](https://r.je/dice.html) parent) different? A lot of DICs require that you provide some configuration for each possible component in order just to work.
...
### 5.1 Using rules to configure shared dependencies
 ...
 The method of defining shared objects is by Rules. See the section on [Rules](http://redcatphp.com/strategy-dependency-injection#config-rules) below for more information. They are used to configure the container. Here's how a shared object is defined using a rule.
...
```
 ...
 The *construct* rule has been added to ensure that every time an instance of PDO is created, it's given a set of constructor arguments. See the section on [construct](http://redcatphp.com/strategy-dependency-injection#config-rules-constructor) for more information.
...
- **shared** (*boolean*) - Whether a single instance is used throughout the container. [View Example](http://redcatphp.com/strategy-dependency-injection#shared-dependencies-rule)
- **inherit** (*boolean*) - Whether the rule will also apply to subclasses (defaults to true). [View Example](http://redcatphp.com/strategy-dependency-injection#config-rules-inheritance)
- **construct** (*array*) - Additional parameters passed to the constructor. [View Example](http://redcatphp.com/strategy-dependency-injection#config-rules-constructor)
- **substitutions** (*array*) - key->value substitutions for dependencies. [View Example](http://redcatphp.com/strategy-dependency-injection#config-rules-substitutions)
- **call** (*multidimensional array*) - A list of methods and their arguments which will be called after the object has been constructed. [View Example](http://redcatphp.com/strategy-dependency-injection#config-rules-shared)
- **instanceOf** (*string*) - The name of the class to initiate. Used when the class name is not passed to $di->addRule(). [View Example](http://redcatphp.com/strategy-dependency-injection#config-rules-named)
- **shareInstances** (*array*) - A list of class names that will be shared throughout a single object tree. [View Example](http://redcatphp.com/strategy-dependency-injection#config-rules-tree)
...
3. Named instances. See the section on [Named instances](http://redcatphp.com/strategy-dependency-injection#config-rules-named) for a more detailed explanation of how this works.
...
Because B inherits A, rules applied to A will applied to B (this behaviour can be turned off, [see the section on inheritance](http://redcatphp.com/strategy-dependency-injection#config-rules-inheritance)) so in this instance, B will be both shared and have the constructor parameters set.
...
### defineClass
 ...
 You can use this API to automatically interchange constructor params or setters params by name with arbitrary variables setted in Strategy (see [arbitrary data](http://redcatphp.com/strategy-dependency-injection#arbitrary-data)).   
