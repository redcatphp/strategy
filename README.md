...
1. [Notes about Kinetic](Documentation/Kinetic-Dependency-Injection#about)1. [Origin](Documentation/Kinetic-Dependency-Injection#origin)
2. [Differences](Documentation/Kinetic-Dependency-Injection#differences)
2. [Paradigm](Documentation/Kinetic-Dependency-Injection#paradigm)1. [Simplify your code](Documentation/Kinetic-Dependency-Injection#simplify)
...
3. [Get Started](Documentation/Kinetic-Dependency-Injection#get-started)1. [Di instanciation](Documentation/Kinetic-Dependency-Injection#di-instanciation)
2. [Classes instanciation](Documentation/Kinetic-Dependency-Injection#classes-instanciation)
4. [Basic usage](Documentation/Kinetic-Dependency-Injection#basic-usage)1. [Object graph creation](Documentation/Kinetic-Dependency-Injection#basic-usage-1)
2. [Providing additional arguments to constructors](Documentation/Kinetic-Dependency-Injection#basic-usage-2)
5. [Shared dependencies](Documentation/Kinetic-Dependency-Injection#shared-dependencies)1. [Using rules to configure shared dependencies](Documentation/Kinetic-Dependency-Injection#shared-dependencies)
6. [Configuring the container with rules](Documentation/Kinetic-Dependency-Injection#config-rules)1. [Substitutions](Documentation/Kinetic-Dependency-Injection#config-rules-substitutions)
...
8. [Arbitrary Data](Documentation/Kinetic-Dependency-Injection#arbitrary-data)1. [Simple variable definition](Documentation/Kinetic-Dependency-Injection#simple-variable)
...
$a = new A(new B, new C, new D(new E, new F));
        ```
Kinetic way (zero configuration):  ```php
$a = $di->create('A');
        ```
...
Dependency injection way:  ```php
class A {
    private $b;
    private $c;
    private $d;
    function __construct(B $b, C $c, D $d){
        $this->b = $b;
        $this->c = $c;
        $this->d = $d;
    }
...
```php
class B {
    function __construct(PDO $pdo, C $c){
...
    function __construct(B $b, $name, $lastname, $pseudo){
        $this->name = $name;
...
```php
class A {
    function __construct(B $b, $foo, $bar) {
    }
}
        ```
...
```php
class A {
    function __construct($foo, $bar, B $b) {
    }
}
...
```php
class DataCopier {
    function __construct(PDO $database1, PDO $database2) {
        
    }
}
...
var_dump($session===$session2); //will show true
        ```
...
var_dump($session===$session2); //will show false
        ```
...
### loadPhpMap
 ...
 This method is based on the same princile than *loadPhp* but you have to pass it an array of config files. The difference is that all variables defined by *$* will be merged recursively by cascade following the order of files in *map* array before applying rules which will be merged recursively too.  ```php
$di->loadPhpMap([
    '/path/to/default_config.php',
    '/path/to/config.php',
]);
        ```
### load
